...
namespace LoginDemo.Services
{
    ...
    {
        string connectionstring = "Data Source=DESKTOP-NGQAKEI;Initial Catalog=SampleWorkouts;Persist Security Info=True;User ID=kothai;Password=abc123";      
        public async Task<ResponseModel<string>> login(LoginViewModel user)
        {
            ...
            if (user != null)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionstring))
                {
                    ...
                    cmd.CommandText = string.Format("Select * FROM tbl_Users WHERE password = '{0}' and Email='{1}'", user.Password, user.Email);
                    cmd.Connection = conn;
        ...
        public async Task<ResponseModel<string>> Register(LoginViewModel user)
        {
            ...
            if (user != null)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionstring))
                {
                    ...
                    SqlCommand cmd = new SqlCommand();
                    cmd.CommandText = string.Format("Insert INTO tbl_Users(Email,Password,Reg_Date) VALUES('{0}','{1}','{2}')", user.Email, user.Password, date);
                    cmd.Connection = conn;
